...
const darkPalette = (theme) => ({
  ...
  // dark 800 & 900
  dark: {
    light: theme.colors?.darkLevel1,
    main: theme.colors?.darkLevel2
  },
  text: {
    primary: theme.darkTextPrimary,
    secondary: theme.darkTextSecondary,
    disabled: theme.colors?.darkTextDisabled,
    icon: theme.colors?.darkTextIcon
  },
  background: {
...
export default function themePalette(theme) {
  const mode = theme.customization.darkMode;
  return {
    mode,
    ...(mode === 'light' ? lightPalette(theme) : darkPalette(theme))
  };
}